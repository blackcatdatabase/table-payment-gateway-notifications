...
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'transaction_id', 'received_at', 'processing_by', 'processing_until', 'attempts', 'last_error', 'status' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'transaction_id', 'received_at', 'processing_by', 'processing_until', 'attempts', 'last_error', 'status' ], true);
            $parts = [];
