 ...
 */
final class PaymentGatewayNotificationsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PaymentGatewayNotificationRepository $paymentGatewayNotificationRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'payment_gateway_notifications' => $this->paymentGatewayNotificationRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PaymentGatewayNotifications\ModuleException("Record {$idStr} not found for locking.");
            }
