...
namespace BlackCat\Database\Packages\PaymentGatewayNotifications\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PaymentGatewayNotifications\Definitions;
use BlackCat\Database\Packages\PaymentGatewayNotifications\Criteria;
use BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto as Dto;
use BlackCat\Database\Packages\PaymentGatewayNotifications\Mapper\PaymentGatewayNotificationDtoMapper as RowMapper;
...
final class PaymentGatewayNotificationRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PaymentGatewayNotifications\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'received_at', 'processing_by', 'processing_until', 'attempts', 'last_error', 'status' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'transaction_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PaymentGatewayNotifications\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'transaction_id' ], [ 'received_at', 'processing_by', 'processing_until', 'attempts', 'last_error', 'status' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto|null */
    public function getByTenantIdAndTransactionId(int $tenantId, string $transactionId, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'transaction_id' => $transactionId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PaymentGatewayNotifications\Mapper\PaymentGatewayNotificationDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\PaymentGatewayNotifications\Dto\PaymentGatewayNotificationDto|null {
