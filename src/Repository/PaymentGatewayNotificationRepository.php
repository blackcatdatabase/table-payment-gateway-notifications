...
use BlackCat\Database\Packages\PaymentGatewayNotifications\Repository\PaymentGatewayNotificationRepositoryInterface;
...
class PaymentGatewayNotificationRepository implements PaymentGatewayNotificationRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\PaymentGatewayNotifications\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'received_at', 'processing_by', 'processing_until', 'attempts', 'last_error', 'status' ]);
          return count($rows);
        }
