...
namespace BlackCat\Database\Packages\PaymentGatewayNotifications\Joins;
 ...
 */
final class PaymentGatewayNotificationsJoins {
    ...
    /**
     * FK: payment_gateway_notifications -> tenants
    ...
    /**
     * FK: payment_gateway_notifications -> payments
     * LEFT JOIN vw_payments AS $as ON $as.tenant_id = $alias.tenant_id AND $as.transaction_id = $alias.transaction_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinPayments(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_payments AS ' . $as . ' ON ' . $as . '.tenant_id = ' . $alias . '.tenant_id' . ' AND ' . $as . '.transaction_id = ' . $alias . '.transaction_id' . ' ', []];
    }
